...
| Cognito's built-in email sender | Free, and enough for onboarding in batches. SES arrives with confirmations in `05-waitlist` and can be passed down then | Wiring SES here, before anything else needs it |
...
- No SES. Cognito's built-in sender is capped at 50 emails a day, which is under
  ...
  to the module once `05-waitlist` brings SES.
