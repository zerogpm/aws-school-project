...
resource "terraform_data" "site_content" {
  ...
  triggers_replace = {
    ...
    # Same reasoning for the API: its URL is compiled into the bundle, so a
    # recreated API has to force a rebuild or the site keeps calling a hostname
    # that no longer resolves.
    api_url = module.booking.api_url
  }
  ...
  provisioner "local-exec" {
    ...
    environment = {
      ...
      # Read by the front end from episode 04 onward, when the booking form
      # stops being a prop. Passed from this stage because the API exists from
      # this stage, and a build that has it is one less thing to redo later.
      VITE_API_URL = module.booking.api_url
    }
  }
}
